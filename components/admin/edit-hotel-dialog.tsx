...
import { updateHotelSchema, type UpdateHotelFormData } from '@/lib/validations'
...
export function EditHotelDialog({ open, onOpenChange, hotel, onSuccess }: EditHotelDialogProps) {
  ...
  const {
    ...
    defaultValues: {
      name: hotel.name,
      atoll: hotel.atoll,
      type: hotel.type as 'resort' | 'city_hotel' | 'guesthouse',
      staff_count: hotel.staff_count,
      status: hotel.status as 'active' | 'closed',
    },
  })
  ...
  // Update form values when hotel changes
  useEffect(() => {
    if (hotel) {
      setValue('name', hotel.name)
      setValue('atoll', hotel.atoll)
      setValue('type', hotel.type as 'resort' | 'city_hotel' | 'guesthouse')
      setValue('staff_count', hotel.staff_count)
      setValue('status', hotel.status as 'active' | 'closed')
    }
  }, [hotel, setValue])
        ...
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription>
                Update hotel, resort, or guesthouse details
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
